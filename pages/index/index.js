...
Page({
  ...
  onLoad: function (options) {
    ...
    // 地址数据加载。。。。。。
    if (options.location){
      that.setData({ 'locationStr': options.location })
  ...
  filterTitleSelectedAtIndex0: function (event) {
    ...
    if (this.data.taskSelectedIndex  !==  index) {
      ...
      page = 1;
      hadLastPage = false;
      this.setData({ homeList: [], reachBottomTip: false, taskHiden: true});//将数据清空
  ...
  filterTitleSelectedAtIndex1: function (event) {
    ...
    if (this.data.areaSelectedIndex !== index) {
      ...
      page = 1;
      hadLastPage = false;
      this.setData({ homeList: [], reachBottomTip: false, areaHiden: true});//将数据清空
      this.loadScoreListFun(this.data.searchVal, this.data.taskval, this.data.areaval, this.data.sortval); 
    }
    this.setData({
      filterDropDownListHiden: true
  ...
  filterTitleSelectedAtIndex2: function (event) {
    ...
    if (this.data.sortSelectedIndex !== index) {
      ...
      page = 1;
      hadLastPage = false;
      this.setData({ homeList: [], reachBottomTip: false, sortHiden: true});//将数据清空
      this.loadScoreListFun(this.data.searchVal, this.data.taskval, this.data.areaval, this.data.sortval); 
    }
    this.setData({
      filterDropDownListHiden: true
  ...
  loadScoreListFun: function (searchVal,subtype,area,sort){
    ...
    wx.request({
      ...
      data: { 'limit': 10, 'page': page, 'status': 4, 'activityname': searchVal, 'subtype': subtype, 'area': area, 'sort': sort, 'lat': app.globalData.latitude, 'lng': app.globalData.longitude},
  ...
  skip_bannerDetail: function (e) {
    ...
    console.log(bannerType); console.log(bannerRequesturl);
      if(bannerType==1){
        wx.navigateTo({
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (res.data.code == 0) {
          ...
          console.log(data)
          if (data && data.length > 0) {
            that.setData({ 'sysMessage': data[0].message });
          }
          //that.setData({ 'homeList': homeList });
        };
